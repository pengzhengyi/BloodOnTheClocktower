...
import type { Prioritization } from '../types/prioritization';
import type { TRandomRange } from '../types/random';
import type { AsyncReducer, Reducer } from '../types/reducer';
import type { Task } from '../types/task';
import type { AnyTransform, Transform } from '../types/transform';
import type { RecoverableException } from '../utils/exception';
import { isIterable } from '../utils/is';
import { getRandomCryptoIntInclusive } from '../utils/random';
import { DefaultDict } from './default-dict';
import { LimitTaskQueue, TaskQueue } from './task-queue';
...
export class Generator<T> implements Iterable<T> {
    ...
    static toPromise<T1, TError extends RecoverableException, T2 = T1>(
    ...
    static catch<
        T,
        TError extends RecoverableException,
        TErrorClassType extends typeof RecoverableException
    >(
    ...
    static chain<T = unknown>(...iterables: Array<Iterable<T>>): Iterable<T> {
        return this.chainFromIterable(iterables);
    }

    static *chainFromIterable<T = unknown>(
    ...
    static *chainFromObjectValues<T = unknown>(
        object: Record<string, Iterable<T>> | any
    ): Iterable<T> {
        return this.chainFromIterable<T>(Object.values<Iterable<T>>(object));
    }
    ...
    *[Symbol.iterator](): Iterator<T> {
    ...
    cartesianProduct<T2>(otherIterable: Iterable<T2>) {
        return this.become((iterable) =>
            Generator.cartesianProduct(iterable, otherIterable)
        );
    }
    ...
    chainFromIterable<T2 = T>(iterables: Iterable<Iterable<T2>>) {
        return this.become((iterable) =>
            Generator.chainFromIterable<T | T2>(
                (function* () {
                    yield iterable;
                    yield* iterables;
                })()
